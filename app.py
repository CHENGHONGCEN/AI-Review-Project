...
def citation_inclusion_prompt_note(
    ...
    return "\n\n".join(
        [
            "AI literature screening marking was run in batches to avoid token-per-minute limits.",
...
def build_screening_excel_export(
    ...
    methodology_sheet.append(["Record ID note", "Record ID is generated by this app during RIS/NBIB import for audit matching. DOI and PMID are parsed from the source file when available."])
...
def render_header() -> None:
    st.markdown(
        ...
        <div class="app-hero">
            ...
            <h1>Literature Screening, Evidence Extraction & Quality Appraisal</h1>
...
def render_citation_inclusion_prompt_editor_content() -> str:
    ...
    st.markdown(
        '<div class="section-note">Edit the prompt used for literature screening against your inclusion and exclusion criteria. The placeholders are filled automatically before the title and abstract are sent to the model.</div>',
        unsafe_allow_html=True,
    )
    st.button(
        "Restore default literature screening prompt",
        key="restore_citation_inclusion_prompt",
        help="Reset the literature screening prompt template to the built-in default.",
        on_click=restore_default_citation_inclusion_prompt,
    )
    prompt_template = st.text_area(
        "Literature screening prompt template",
        value=saved_value,
        height=360,
        help="Keep {inclusion_criteria}, {exclusion_criteria}, and {records_json} if you want the app to insert the current criteria and literature records at those positions.",
...
def render_citation_inclusion_prompt_editor() -> str:
    with st.expander("Literature screening prompt", expanded=False):
...
def render_prompt_settings() -> tuple[str, str, str]:
    ...
    with st.expander("Prompt settings", expanded=False):
        ...
        if st.session_state.get("prompt_editor_choice") in {"Citation exclusion", "Citation inclusion"}:
            st.session_state.prompt_editor_choice = "Literature Screening"
        prompt_choice = st.selectbox(
            "Prompt to edit",
            ["Literature Screening", "Article extraction", "MMAT assessment"],
            key="prompt_editor_choice",
            label_visibility="collapsed",
        )
        if prompt_choice == "Literature Screening":
...
def render_results() -> None:
    ...
    if st.session_state.results or st.session_state.qa_results:
        export_bytes = export_service.build_excel_export(
            st.session_state.results,
            st.session_state.qa_results,
            result_to_flat_row=result_to_flat_row,
            result_to_evidence_rows=result_to_evidence_rows,
    ...
    if st.session_state.results:
        ...
        if uploaded_pdf_bytes:
            highlighted_zip, highlight_report = pdf_highlighting.build_highlight_zip(
                st.session_state.results,
                uploaded_pdf_bytes,
            )
            highlighted_count, total_count = pdf_highlighting.summarize_report(highlight_report)
            if total_count:
                st.caption(
                    f"{highlighted_count} of {total_count} evidence excerpts highlighted; "
                    "see highlight_report.csv in the zip for unmatched excerpts."
                )
            st.download_button(
                "Download highlighted PDFs zip",
                data=highlighted_zip,
                file_name=f"highlighted_pdfs_{datetime.now().strftime('%Y%m%d_%H%M')}.zip",
                mime="application/zip",
            )
        else:
            st.info("Re-upload the source PDFs to generate highlighted PDF downloads.")
    ...
    for index, uploaded_file in enumerate(uploaded_files, start=1):
        ...
        try:
            result = extraction_service.extract_from_pdf(
                ...
                normalize_result=normalize_extraction_result,
    ...
    for index, uploaded_file in enumerate(uploaded_files, start=1):
        ...
        try:
            result = extraction_service.assess_quality_from_pdf(
                ...
                mmat_schema=MMAT_SCHEMA,
                normalize_result=normalize_mmat_result,
                default_prompt_template=DEFAULT_MMAT_PROMPT_TEMPLATE,
                official_rubric=OFFICIAL_MMAT_2018_RUBRIC,
                mmat_manual_version=MMAT_MANUAL_VERSION,
            )
